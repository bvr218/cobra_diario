    ...
    @if($showModal)
        <div class="modal-backdrop" wire:click="cerrarModal">
            <div class="modal-content" @click.stop x-data="{ print() { window.print(); } }">
                ...
                <div id="printable-prestamos-content" class="printable-area">
                            ...
                            <tbody>
                                @forelse($lista as $item)
                                    <tr class="table-row-hoverable">
                                        <td>{{ $item['posicion_ruta'] ?? 0 }}</td>
                                        <td>{{ $item['cliente_nombre'] ?? 'N/A' }}</td>
                                        <td>${{ number_format($item[$valorCampo] ?? 0, 0, ',', '.') }}</td>
                                        <td>
                                            <span class="px-2 py-1 text-xs font-semibold rounded-full
                                                {{ ($item['estado'] ?? '') === 'autorizado' ? 'bg-blue-100 text-blue-800' :
                                        ...
                                        <td>{{ $item['cliente_descripcion'] ?? 'N/A' }}</td>
                                    </tr>
                                @empty
                                    <tr><td colspan="6" class="modal-empty-message">No hay datos.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
