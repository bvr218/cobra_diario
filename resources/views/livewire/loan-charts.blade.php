<div> {{-- 1. Nuevo contenedor raíz. NO tiene wire:ignore. --}}
    <div
        ...
        x-data="{
            ...
            window.Livewire.on('chartDataUpdated', (detail) => {
                initPie(detail.pieChartData || detail, this.$wire);
            });
    ...
    >
        <div class="flex flex-col lg:flex-row gap-10">
            ...
            <div class="w-full lg:w-1/2">
                <h3 class="text-base sm:text-lg font-semibold text-center mb-4 text-gray-900 dark:text-gray-100">
                    Evolución en Caja ({{ $nombreMesActual }})
                </h3>
    ...
    {{-- 3. El modal ahora está fuera del bloque wire:ignore y usa clases CSS personalizadas. --}}
    <div class="list-modal-wrapper">
        ...
        @if ($showLoanListModal)
            <div class="chart-modal-backdrop"
                 x-data @mousedown.outside="if ($event.target.classList.contains('chart-modal-backdrop')) { $wire.closeLoanListModal() }">
                <div class="chart-modal-content">
                    {{-- Encabezado del Modal --}}
                    <div class="chart-modal-header">
                        <h3 class="chart-modal-title">{{ $modalTitle }}</h3>
                        <button wire:click="closeLoanListModal" class="chart-modal-close-button">×</button>
                    </div>

                    {{-- Cuerpo del Modal (Tabla) --}}
                    <div class="chart-modal-body">
                        @if($loanList->isNotEmpty())
                                <table class="chart-modal-table">
                                    <thead class="table-header-sticky">
                                        <tr>
                                            <th class="chart-modal-th">Cliente</th>
                                            <th class="chart-modal-th">Fecha de Pago</th>
                                            <th class="chart-modal-th">Deuda Inicial</th>
                                            <th class="chart-modal-th">Deuda Actual</th>
                                            
                                            @if($currentListType !== 'alDia')
                                            <th class="chart-modal-th">Valor Cuota</th>
                                            @endif

                                            @if($currentListType === 'alDia')
                                            <th class="chart-modal-th">Última Cuota Recibida</th>
                                            @endif

                                        </tr>
                                    </thead>
                                    <tbody class="chart-modal-tbody">
                                        @foreach($loanList as $loan)
                                            @php
                                                $url = $canViewPrestamos ? PrestamoResource::getUrl('edit', ['record' => $loan]) : '#';
                                            @endphp
                                            <tr @if($canViewPrestamos) class="chart-modal-tr-clickable" onclick="window.location='{{ $url }}'" @endif>
                                                <td class="chart-modal-td">
                                                    <a href="{{ $url }}" class="{{ $canViewPrestamos ? 'chart-modal-link' : 'chart-modal-text-strong' }}">
                                                        {{ $loan->cliente->nombre ?? 'N/A' }}
                                                    </a>
                                                </td>
                                                <td class="chart-modal-td chart-modal-text-secondary">
                                                    {{ $loan->next_payment ? $loan->next_payment->format('d/m/Y') : 'N/A' }}
                                                </td>
                                                <td class="chart-modal-td chart-modal-text-secondary">${{ number_format($loan->deuda_inicial, 0) }}</td>
                                                <td class="chart-modal-td chart-modal-text-secondary">${{ number_format($loan->deuda_actual, 0) }}</td>

                                                @if($currentListType !== 'alDia')
                                                <td class="chart-modal-td chart-modal-text-secondary">${{ number_format($loan->monto_por_cuota, 0) }}</td>
                                                @endif
                                                
                                                @if($currentListType === 'alDia')
                                                    @php $ultimoAbono = $loan->abonos->sortByDesc('created_at')->first(); @endphp
                                                    <td class="chart-modal-td chart-modal-text-secondary">
                                                        {{ $ultimoAbono ? '$' . number_format($ultimoAbono->monto_abono, 0) : 'N/A' }}
                                                    </td>
                                                @endif
                                    ...
                                    <tfoot class="table-footer-sticky">
                                        <tr>
                                            <td>Totales</td>
                                            <td></td> {{-- Celda vacía para alinear con la columna 'Fecha de Pago' --}}
                                            <td>${{ number_format($totalDeudaInicial, 0) }}</td>
                                            <td>${{ number_format($totalDeudaActual, 0) }}</td>
                                            
                                            @if($currentListType !== 'alDia')
                                            <td>${{ number_format($totalValorCuota, 0) }}</td>
                                            @endif

                                            @if($currentListType === 'alDia')
                                            <td>${{ number_format($totalUltimaCuota, 0) }}</td>
                                            @endif
                        ...
                        @else
                            <p class="chart-modal-empty-message">No hay préstamos para mostrar en esta categoría.</p>
                        @endif
                    </div>

                    {{-- Pie del Modal --}}
                    <div class="chart-modal-footer">
                        <button wire:click="closeLoanListModal" type="button" class="chart-modal-footer-button">
                            Cerrar
                        </button>
                    </div>
                </div>
