    ...
    @if($showModal)
        <div class="modal-backdrop" wire:click="cerrarModal">
            <div class="modal-content" @click.stop x-data="{ print() { window.print(); } }">
                ...
                <div id="printable-finalizados-content" class="printable-area">
                            ...
                            <tbody>
                                @forelse($lista as $item)
                                    <tr class="table-row-hoverable">
                                        ...
                                        <td>${{ number_format($item['deuda_inicial'] ?? 0, 0, ',', '.') }}</td>
                                        <td>${{ number_format($item['deuda_actual'] ?? 0, 0, ',', '.') }}</td>
                                        <td>{{ $item['fecha_ultimo_movimiento'] ?? 'N/A' }}</td>
                                    </tr>
                                @empty
                                    <tr><td colspan="4" class="modal-empty-message">No se encontraron préstamos finalizados.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
