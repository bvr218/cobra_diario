    ...
    @if($showModal)
        <div class="modal-backdrop" wire:click="cerrarModal">
            <div class="modal-content" @click.stop x-data="{ print() { window.print(); } }">
                ...
                <div id="printable-refinanciaciones-content" class="printable-area">
                            ...
                            <tbody>
                                @forelse($lista as $item)
                                    <tr class="table-row-hoverable">
                                        ...
                                        <td>${{ number_format($item['deuda_anterior'] ?? 0, 0, ',', '.') }}</td>
                                        <td>${{ number_format($item['valor'] ?? 0, 0, ',', '.') }}</td>
                                        <td>${{ number_format($item['deuda_refinanciada_interes'] ?? 0, 0, ',', '.') }}</td>
                                        <td>
                                             <span class="px-2 py-1 text-xs font-semibold rounded-full
                                                {{ ($item['estado'] ?? '') === 'autorizado' ? 'bg-green-100 text-green-800' :
                                ...
                                @empty
                                    <tr><td colspan="6" class="modal-empty-message">No se encontraron refinanciaciones.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
