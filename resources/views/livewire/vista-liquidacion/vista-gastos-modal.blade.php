    ...
    @if($showModal)
        <div class="modal-backdrop" wire:click="cerrarModal">
            <div class="modal-content" @click.stop x-data="{ print() { window.print(); } }">
                ...
                <div id="printable-gastos-content" class="printable-area">
                            ...
                            <tbody>
                                @forelse($lista as $item)
                                    <tr class="table-row-hoverable">
                                        ...
                                        <td>{{ $item['informacion'] ?? 'N/A' }}</td>
                                        <td>{{ $item['created_at'] ?? 'N/A' }}</td>
                                        <td>
                                            <span class="px-2 py-1 text-xs font-semibold rounded-full
                                                {{ ($item['autorizado'] ?? false) ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                {{ ($item['autorizado'] ?? false) ? 'Sí' : 'No' }}
                                            </span>
                                        </td>
                                    </tr>
                                @empty
                                    <tr><td colspan="6" class="modal-empty-message">No se encontraron gastos.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
